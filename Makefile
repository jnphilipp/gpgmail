...
build/changelog.Debian.gz: build
	$(Q)declare TAGS=(`git tag`); for ((i=$${#TAGS[@]};i>=0;i--)); do if [ $$i -eq 0 ]; then echo -e "gpgmail ($${TAGS[$$i]}) unstable; urgency=medium" >> build/changelog; git log $${TAGS[$$i]} --no-merges --format="  * %h %s"  >> build/changelog; git log $${TAGS[$$i]} -n 1 --format=" -- %an <%ae>  %aD" >> build/changelog; elif [ $$i -eq $${#TAGS[@]} ] && [ $$(git log $${TAGS[$$i-1]}..HEAD --oneline | wc -l) -ne 0 ]; then echo -e "gpgmail $${i} ($${TAGS[$$i-1]}-$$(git log -n 1 --format='%h')) unstable; urgency=medium" >> build/changelog; git log $${TAGS[$$i-1]}..HEAD --no-merges --format="  * %h %s"  >> build/changelog; git log HEAD -n 1 --format=" -- %an <%ae>  %aD" >> build/changelog; elif [ $$i -lt $${#TAGS[@]} ]; then echo -e "gpgmail ($${TAGS[$$i]}) unstable; urgency=medium" >> build/changelog; git log $${TAGS[$$i-1]}..$${TAGS[$$i]} --no-merges --format="  * %h %s"  >> build/changelog; git log $${TAGS[$$i]} -n 1 --format=" -- %an <%ae>  %aD" >> build/changelog; fi; done
